...
struct andr_boot_info_t {
    ...
    u32 boot_ramdisk_addr; /* physical load addr */
    u32 vendor_ramdisk_addr; /* physical load addr */
    ...
    char cmdline[TOTAL_BOOT_ARGS_SIZE];

    u32 dtb_size; /* size in bytes for DTB image */
    u64 dtb_addr; /* physical load address for DTB image */
} __attribute__((packed));

typedef struct andr_boot_info_t andr_boot_info;
